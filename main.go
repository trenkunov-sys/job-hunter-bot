...
func callDeepSeek(apiKey, prompt string) (string, error) {
    ...
    reqBody := map[string]interface{}{
        "model": "deepseek-chat", "temperature": 0.7, "max_tokens": 600,
...
func (s *JobSearcher) searchHimalayas(profession string) []Job {
    ...
    var result struct { Jobs []struct {
        Title, CompanyName, Location, Description, URL string; SalaryMin, SalaryMax int; Currency string
    }`json:"jobs"` }
    ...
    for _, j := range result.Jobs {
        ...
        company := j.CompanyName; if company == "" { company = "Himalayas" }
        ...
        url := j.URL; if url == "" { url = "https://himalayas.app" }
        jobs = append(jobs, Job{Title: j.Title, Company: company, Location: j.Location, Description: desc, URL: url, Salary: salary, Source: "Himalayas", Stack: stack})
    }
    return jobs
}
...
func (s *JobSearcher) searchAdzuna(profession, country string) []Job {
    ...
    for _, j := range result.Results {
        ...
        company := j.Company.DisplayName; if company == "" { company = "Adzuna" }
        desc := stripHTML(j.Description)
        if len(desc) > 400 { desc = desc[:400] + "..." }
        if desc == "" { desc = "Описание недоступно" }
        stack := extractStack(j.Description)
        url := j.RedirectURL; if url == "" { url = "https://adzuna.com" }
        jobs = append(jobs, Job{Title: j.Title, Company: company, Location: j.Location.DisplayName, Description: desc, URL: url, Salary: salary, Source: "Adzuna", Stack: stack})
    }
    return jobs
}
...
func (s *JobSearcher) getMockJobs(profession, country string) []Job {
    return []Job{
        {Title: fmt.Sprintf("Senior %s", profession), Company: "Tech Corp", Location: country, Description: "Разработка систем.", URL: "https://example.com", Source: "Mock", Salary: "$80,000 - $100,000", Stack: "Go, PostgreSQL"},
        {Title: profession, Company: "Startup Inc", Location: country, Description: "Agile-команда.", URL: "https://example.com", Source: "Mock", Salary: "$60,000 - $80,000", Stack: "Python, Django"},
    }
}
...
func generateCoverLetter(apiKey, cvText, jobTitle, company, jobDescription string) string {
...
Письмо должно:
1. Начинаться с обращения к hiring-менеджеру
2. Показать, что кандидат изучил компанию и вакансию
3. Связать конкретный опыт из резюме с требованиями вакансии (1-2 конкретных примера)
4. Быть убедительным, но не длинным (4-6 предложений)
5. Заканчиваться призывом к действию`, cvText, jobDescription)
    resp, err := callDeepSeek(apiKey, prompt)
    if err != nil { return fmt.Sprintf("Здравствуйте!\n\nЯ откликаюсь на вакансию %s в %s.\n\nМой опыт соответствует требованиям.\n\nС уважением,\nКандидат", jobTitle, company) }
    return resp
}

func formatJobCard(job Job) string {
    var sb strings.Builder
    sb.WriteString(fmt.Sprintf("<b>%s</b> [%s] @ %s\n\n", job.Title, job.Source, job.Company))
    if job.Salary != "" { sb.WriteString(fmt.Sprintf("💰 <b>%s</b>\n\n", job.Salary)) }
    if job.Location != "" { sb.WriteString(fmt.Sprintf("📍 %s\n\n", job.Location)) }
    if job.Stack != "" && job.Stack != "Не указан" { sb.WriteString(fmt.Sprintf("🛠 Стек: %s\n\n", job.Stack)) }
    sb.WriteString(fmt.Sprintf("📝 %s\n\n", job.Description))
    sb.WriteString(fmt.Sprintf("🔗 <a href=\"%s\">Открыть вакансию</a>", job.URL))
    return sb.String()
}
...
func processUpdate(cfg *Config, upd Update) {
    ...
    if upd.CallbackQuery != nil {
        ...
        case "apply":
            letter := generateCoverLetter(cfg.DeepSeekAPIKey, s.CVText, job.Title, job.Company, job.Description)
    ...
    if strings.HasPrefix(msg.Text, "/") {
        ...
        case "/how": sendMessage(token, chatID, "📋 Загрузи PDF → выбери страну → получи вакансии → нажми «Откликнуться». Бот напишет письмо, отклик отправишь сам по ссылке.", mainKeyboard())
