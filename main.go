...
func (s *JobSearcher) searchHimalayas(profession string) []Job {
    ...
    defer resp.Body.Close()
    body, _ := io.ReadAll(resp.Body)
    log.Printf("Himalayas response: %s", string(body[:min(500, len(body))]))
    var result struct {
        Jobs []struct {
            ID          string `json:"id"`
            Title       string `json:"title"`
...
func (s *JobSearcher) searchAdzuna(profession, country string) []Job {
    ...
    defer resp.Body.Close()
    body, _ := io.ReadAll(resp.Body)
    log.Printf("Adzuna response: %s", string(body[:min(500, len(body))]))
    var result struct {
        Results []struct {
            Title       string `json:"title"`
...
func processUpdate(cfg *Config, upd Update) {
    ...
    if upd.CallbackQuery != nil {
        ...
        case "apply":
            ...
            sendMessage(token, chatID, fmt.Sprintf("📧 *Письмо для %s в %s:*\n\n%s\n\n🔗 [Ссылка на вакансию](%s)\n\n⚠️ Отклик отправьте самостоятельно по ссылке.", job.Title, job.Company, letter, job.URL), nil)
            session.JobIndex++
            showCurrentJob(token, session)
        }
        return
...
✉️ Отклик: Я сгенерирую письмо, а отправить отклик нужно самому по ссылке.

📋 Команды:
/start — начать
/reset — сбросить
/how — как работает
/feedback — связь с @Trene4ca

Отправь PDF с резюме:`, username)

            sendMessage(token, chatID, welcomeText, removeKeyboard())

        case "/how":
            sendMessage(token, chatID, `📋 Как работает бот:

1️⃣ Загрузите PDF с резюме
2️⃣ Выберите страну
3️⃣ Получите реальные вакансии с описанием
4️⃣ Нажмите «Откликнуться» — бот напишет письмо
5️⃣ Отправьте отклик сами по ссылке

⚠️ Бот НЕ отправляет отклики автоматически из-за CAPTCHA на сайтах.`, mainKeyboard())
