...
func (s *JobSearcher) getMockJobs(profession, country string) []Job {
    return []Job{
        {Title: profession, Company: "Demo Company", Location: country, Description: "Тестовая вакансия", URL: "#", HasTest: false, Source: "Mock"},
    }
}
...
func processUpdate(cfg *Config, upd Update) {
    ...
    if strings.HasPrefix(msg.Text, "/") {
        ...
        case "/start":
            sessionMutex.Lock()
            userSessions[chatID] = &UserSession{State: "awaiting_cv", LastActivity: time.Now()}
            sessionMutex.Unlock()
    ...
    if msg.Text != "" {
        ...
        case "awaiting_profession":
            sessionMutex.Lock()
            session.Profession = strings.TrimPrefix(msg.Text, "✅ ")
...
func showCurrentJob(token string, session *UserSession) {
    if session.JobIndex >= len(session.CurrentJobs) {
        sendMessage(token, 0, "🏁 Всё просмотрено!", mainKeyboard())
        return
    }
    ...
    sendMessage(token, 0, text, jobActionKeyboard(session.JobIndex))
}
