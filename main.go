...
func detectProfession(cvText string) string {
    ...
    keywords := map[string][]string{
        ...
        "QA Engineer":      {"qa", "testing", "selenium"},
        "System Analyst":   {"system analyst", "системный аналитик", "use case", "api"},
...
func (s *JobSearcher) searchHimalayas(profession string) []Job {
    apiURL := fmt.Sprintf("https://himalayas.app/jobs/api?search=%s&limit=5", profession)
...
func (s *JobSearcher) searchAdzuna(profession, country string) []Job {
    ...
    apiURL := fmt.Sprintf("https://api.adzuna.com/v1/api/jobs/%s/search/1?app_id=%s&app_key=%s&what=%s",
        countryCode, s.adzunaAppID, s.adzunaAppKey, profession)
    resp, err := http.Get(apiURL)
    if err != nil {
        return nil
    }
...
func (a *AIService) generateCoverLetter(userID int64, cvText, jobTitle, company string) string {
    if a.deepseekKey == "" {
...
func webhookHandler(token string, jobSearcher *JobSearcher, aiService *AIService) http.HandlerFunc {
    return func(w http.ResponseWriter, r *http.Request) {
        ...
        go processUpdate(&Config{TelegramToken: token}, jobSearcher, aiService, upd)
...
func main() {
    ...
    http.HandleFunc("/", webhookHandler(cfg.TelegramToken, jobSearcher, aiService))
    ...
    go func() {
        http.Get(fmt.Sprintf("%s%s/deleteWebhook?drop_pending_updates=false", telegramAPI, cfg.TelegramToken))
        var offset int64 = 0
...
func processUpdate(cfg *Config, jobSearcher *JobSearcher, aiService *AIService, upd Update) {
    ...
    if upd.CallbackQuery != nil {
        ...
        case "apply":
            ...
            letter := aiService.generateCoverLetter(chatID, session.User.CVText, job.Title, job.Company)
...
func showCurrentJob(token string, session *UserSession) {
    ...
    text := fmt.Sprintf("*%d/%d*  %s\n🏢 %s\n📍 %s\n💰 %s",
        session.JobIndex+1, len(session.CurrentJobs), job.Title, job.Company, job.Location, job.Salary)
